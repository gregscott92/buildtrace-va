...
export default function VaScreen() {
  ...
  async function pickImage() {
    ...
    if (!res.canceled && res.assets?.length) {
      ...
      setDebugText(`Selected image: ${asset.uri}`);
    }
  }
  ...
  async function handleAnalyze() {
    ...
    try {
      setLoading(true);
      setResult(null);
      setDebugText("Preparing request...");

      let imageBase64 = "";

      if (imageUri) {
        setDebugText(`Reading image from URI...\n${imageUri}`);
        ...
        if (!rawBase64 || !rawBase64.trim()) {
          throw new Error("Image was selected but base64 conversion returned empty.");
        }
