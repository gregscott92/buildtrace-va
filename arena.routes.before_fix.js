...
async function generateArenaAnswer(post) {
  if (!openai) { console.log("ARENA DEBUG: openai missing"); return null; }

  console.log("ARENA DEBUG: calling openai");
...
`
  ...
  console.log("ARENA DEBUG: raw response =", JSON.stringify(response, null, 2));
  const text = response?.choices?.[0]?.message?.content || "";

  console.log("ARENA DEBUG: extracted text =", text);
  return text;
});
...
const analysis = await analyzeCfr38(input);
    ...
    generated = [
      ...
      "",
      "Why:",
      generated || "VA analysis unavailable",
      "",
      "What You're Missing:",
