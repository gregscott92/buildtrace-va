...
app.post("/signup", async (req, res) => {
  try {
    ...
    console.log("SIGNUP HIT:", { email, hasPassword: !!password });
...
app.post("/analyze-image", async (req, res) => {
  try {
    ...
    const prompt = [
      "You are helping estimate a VA claim based on uploaded paperwork.",
      "Read the image && extract only what is visible.",
      "Focus on:",
      "- diagnosis || condition",
      "- symptom frequency",
      "- whether attacks are prostrating / require lying down / dark room",
      "- duration",
      "- work impact / economic impact",
      "- medications / treatment",
      "- medical opinions / nexus language",
      "",
      "Then return plain text in this exact shape:",
      "Document Summary:",
      "- ...",
      "",
      "Estimated VA Rating:",
    ...
    const result =
      ...
      "Document processed, but no text result was returned.";

    const { error: insertError } = await supabase
      .from("va_claims")
      .insert({
        user_id: null,
        input_text: `[UPLOAD] ${filename || "paperwork image"}`,
        result_text: result
      });

    if (insertError) {
      console.log("VA IMAGE SAVE ERROR:", insertError);
    }
